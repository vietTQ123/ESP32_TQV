...
void TFT_Screen()
{
    if (Serial.available()) // Kiá»ƒm tra xem cÃ³ dá»¯ liá»‡u nÃ o trÃªn cá»•ng Serial khÃ´ng
    {
        String jsonMessage = Serial.readStringUntil('\n'); // Äá»c dá»¯ liá»‡u JSON tá»« Serial cho Ä‘áº¿n khi gáº·p kÃ½ tá»± xuá»‘ng dÃ²ng (\n)

        StaticJsonDocument<200> doc; // Táº¡o má»™t Ä‘á»‘i tÆ°á»£ng JSON Ä‘á»ƒ lÆ°u trá»¯ dá»¯ liá»‡u

        DeserializationError error = deserializeJson(doc, jsonMessage); // Giáº£i mÃ£ dá»¯ liá»‡u JSON tá»« chuá»—i

        if (error) // Kiá»ƒm tra xem cÃ³ lá»—i trong quÃ¡ trÃ¬nh giáº£i mÃ£ khÃ´ng
        {
